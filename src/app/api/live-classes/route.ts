...
export async function POST(request: NextRequest) {
  try {
    ...
    await notifyCourseStudents(validatedData.course, {
      type: "live_class_scheduled",
      title: `Live class scheduled for ${course.name}`,
      link: `/live-stream/${liveClass._id.toString()}`,
      data: { liveClassId: liveClass._id.toString() },
    }).catch((error) => console.error("Live class notification error:", error))
